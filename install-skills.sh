#!/bin/bash
# Install flutter_meta_wearables_dat AI development config into your project.
#
# Usage:
...
#   curl -sL ...install-skills.sh | bash -s claude # Specific tool

set -euo pipefail

REPO="rodcone/flutter_meta_wearables_dat"
BRANCH="main"
ARCHIVE_URL="https://github.com/${REPO}/archive/refs/heads/${BRANCH}.tar.gz"
EXTRACT_DIR="flutter_meta_wearables_dat-${BRANCH}"
...
safe_cleanup() {
  ...
  if [[ ! "$EXTRACT_DIR" =~ ^flutter_meta_wearables_dat- ]]; then
...
download_archive() {
  if [ ! -d "${EXTRACT_DIR}" ]; then
    echo "Downloading config from ${REPO}..."
...
warn_overwrite() {
  local target="$1"
  if [ -e "$target" ]; then
    echo "  ⚠ Existing $target will be overwritten."
  fi
}
...
install_claude() {
  ...
  download_archive
  if [ -d "${EXTRACT_DIR}/agent/claude" ]; then
    warn_overwrite ".claude/"
    mkdir -p .claude
    cp -R "${EXTRACT_DIR}/agent/claude/." .claude/
    echo "  ✓ Installed .claude/ with $(find .claude -name '*.md' -o -name '*.json' | wc -l | tr -d ' ') files."
  else
...
install_copilot() {
  echo "Installing GitHub Copilot config..."
  download_archive
  if [ -d "${EXTRACT_DIR}/agent/github" ]; then
    warn_overwrite ".github/copilot-instructions.md"
    mkdir -p .github
    cp -R "${EXTRACT_DIR}/agent/github/." .github/
...
install_cursor() {
  ...
  download_archive
  if [ -d "${EXTRACT_DIR}/agent/cursor" ]; then
    warn_overwrite ".cursor/rules/"
    mkdir -p .cursor
    cp -R "${EXTRACT_DIR}/agent/cursor/." .cursor/
...
show_menu() {
  echo ""
  echo "flutter_meta_wearables_dat — AI Config Installer"
  echo "================================================="
  echo ""
