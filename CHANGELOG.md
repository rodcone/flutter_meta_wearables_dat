...
* **The root cause of the freeze that prompted this is not confirmed, and this release does not claim to fix it.** A field report on Bluetooth Classic at 30 fps froze for five minutes with zero events, and again 80 seconds into a later session. The evidence points at the transport rather than the plugin. What this release does is make the next occurrence self-diagnosing: `frameStalled`'s message states whether frames stopped arriving from the SDK or arrived and failed to reach the texture, which is the distinction that was previously unobservable in the field.
