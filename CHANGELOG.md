## 0.5.3
* New debug API `describeDevices()` (iOS only for now): snapshot of the SDK's raw device list — identifier, link state, compatibility — plus the selector's current pick. Distinguishes "no devices discovered" (registration/Bluetooth/manifest) from "device filtered out" (e.g. `deviceUpdateRequired`) when `activeDeviceStream()` stays empty.
* iOS: Fix first-registration device discovery. The plugin's shared `AutoDeviceSelector` is created at registrar time — before the app is registered — and such an instance can stay permanently blind to devices that only surface after registration (`activeDeviceStream()` never reports a device, and `startStreamSession` fails `noEligibleDevice` forever). `restartActiveDeviceMonitoring()` now rebuilds the selector when it reports no active device (and no session is live) and force-restarts the monitoring loops onto the new instance — mirroring Meta's CameraAccess sample, which only creates its selector post-registration.
