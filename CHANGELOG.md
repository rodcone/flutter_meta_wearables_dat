...
* **`paused` is no longer treated as a live stream** (iOS + Android). The DAT SDK exposes no public resume, so teardown + `startStreamSession()` is the only recovery an app has — and the liveness guard was short-circuiting exactly that, handing the caller back its existing frozen texture id without restarting anything.
