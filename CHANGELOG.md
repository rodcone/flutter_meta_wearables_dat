...
* **New `frameStalled` error code: a stream that stops delivering frames is now reported.** Both SDKs can go quiet mid-stream while the stream state stays `streaming` and `errorStream` stays silent — a Bluetooth Classic transport stall is the common trigger. The `Texture` holds its last frame indefinitely and nothing on any channel says so, which left apps to detect it by rasterising frames and diffing pixels: a heuristic that cannot tell a frozen stream from a camera pointed at something motionless. The plugin now watches the frame clock and raises `frameStalled` on `streamSessionErrorStream()` after 3 seconds without a frame while the stream still reports `streaming`. It deliberately does **not** restart — a restart mints a new texture ID, and swapping that under a live widget is the app's call. `startStreamSession()` does now treat a stalled stream as stale, so a restart always yields a fresh texture instead of handing back the frozen one. `paused` is excluded (the SDK drives that itself) as is the deliberate background stop, which already reports `stoppedForBackground`.
* **The silent `raw` freeze was the plugin starving the SDK's buffer pool. Measured on device, fixing it took the freeze from roughly once every 30 seconds to once every 3-4 minutes.** The `raw` path handed Flutter the `CVPixelBuffer` hanging off the SDK's own `CMSampleBuffer` — zero-copy, and wrong. That buffer belongs to a pool inside the SDK, and the texture holds the latest one until the next frame replaces it, plus one more retained by the engine between `copyPixelBuffer()` and the render. Two of the SDK's buffers, held indefinitely, on a pool the plugin does not own and cannot size. When the capture pipeline asks for a free buffer and finds none it stops producing, with no error, no state change and `Stream.state` still `.streaming`. Frames now go through a plugin-owned `CVPixelBufferPool`, so the SDK's buffer is released as soon as the frame is processed. The copy happens after the FPS throttle, so only rendered frames pay for it — about 1.8 MB at 504x896, far less than the HEVC decode the `hvc1` path already did. This ends the zero-copy claim for `raw` on iOS; correctness is worth the memcpy.
* **A residual freeze remains, and it is not this.** Roughly one every 3-4 minutes on the same hardware, down from one every 30 seconds. The plugin's own pool cannot be the cause: no `kCVPixelBufferPoolAllocationThresholdKey` is set, so it grows on demand rather than running dry, and after this fix an SDK buffer is held only for the duration of `processAndSendFrame`. The remaining stalls look like the transport giving up, which is outside this plugin. `frameStalled` reports each one, and an app that restarts on it recovers — the example does exactly that. Anyone hitting it often enough to matter should consider `hvc1`, which does not appear to stall at all.
* **Why it took so long to find.** Three facts pointed at it and each was individually dismissible. `hvc1` never froze, because VideoToolbox decodes into its own buffers and releases the SDK's immediately. Android never froze, because `FrameProcessor` converts I420 into its own reusable bitmap and likewise retains nothing. And the plugin's dispatch queue measured a depth of one frame throughout, which looked like an alibi but was not — queue depth is the backlog, and the problem was the retention.
* **The reporting stays, and it is what found this.** Measured on device (Ray-Ban Display, medium, 30 fps, `raw`), the freeze reproduces within 7-20 seconds and always the same way: frames stop arriving from the SDK while `Stream.state` stays `.streaming` and `errorStream` says nothing. The plugin's own queue is not the cause — in-flight depth peaked at **1** frame throughout, so nothing was backing up and no buffer pool was being starved from this side. Sometimes there is a precursor, inter-frame gaps stretching from ~36 ms to over 200 ms across the final two seconds; sometimes delivery stops from a steady 28 fps with no warning at all. Worth noting alongside it: the SDK delivers roughly 50 fps against a requested 30, a steady 1.7-2.3x, which the FPS throttle discards.
* **iOS: the FPS throttle no longer times on the wall clock.** It compared `Date()` values, so a backward NTP step made the elapsed time negative and every frame was dropped — and because the timestamp is only refreshed on frames that survive the throttle, the pipeline could not recover on its own. Frozen preview, `streaming` state, no error, for as long as the correction lasted. Now `CACurrentMediaTime()`, matching Android's existing `System.nanoTime()`.
* **Both platforms: `startStreamSession(fps: 0)` no longer freezes the preview permanently.** The value was clamped for the SDK's `StreamConfiguration` but not for the plugin's own throttle, where it produced an infinite minimum interval — one frame rendered, then nothing, while `videoFramesStream()` kept delivering. Clamped to 1 fps in both places.
* iOS: frame timing moved into a lock-guarded `FrameLivenessTracker`. It was plain mutable state written on the main actor and read on the SDK's frame queue with no barrier, and the teardown reset was skipped whenever a restart raced the 3 s stop-wait, leaving the new stream running on the old stream's timestamp.
* The every-30-frames diagnostic log now reports arrivals alongside pushes (`arrived: 412, pushed: 205, …`) on both platforms. It previously counted only frames that survived the throttle and sat below its early return, so a wedged throttle silenced it exactly as a dead stream did — reading it could not tell the two apart.
* Docs: `captureStreamFrame` now states plainly that sustained polling is supported. It runs entirely in Dart, invokes no native code and retains no SDK buffer, so it cannot starve the capture pipeline however long it polls; the cost is an offscreen GPU rasterisation plus a CPU readback per call. It is also documented as **not** a freeze detector, for the reason above.
* Example: `frameStalled` joins the recoverable-error set, so the stream screen demonstrates automatic restart-on-stall through the existing retry machinery.
* **Example: migrated to the `UIScene` lifecycle, which iOS 27 requires to launch at all.** Plugin registration and the diagnostics channel moved to `didInitializeImplicitFlutterEngine`, since the implicit engine does not exist yet when the app delegate launches — `window?.rootViewController` is nil there, and the old cast had been failing silently. A `SceneDelegate` now handles scene URL events; the previous `application(_:open:options:)` override invoked `handleUrl` *into* Dart on the plugin's channel, which nothing has ever handled — registration callbacks arrive through `app_links`. The plugin itself needed no change: it already observes lifecycle through `NotificationCenter` rather than the application delegate, for exactly this reason. Documented in the README for host apps hitting the same wall.
