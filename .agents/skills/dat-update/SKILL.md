...
```bash
.Codex/skills/dat-update/scripts/preflight.sh $ARGUMENTS
```
...
Then: `git checkout -b dat-<version>`, and read `AGENTS.md` (accurate as of the *previous* release)
...
3. **Docs sweep** — always the under-scoped step:
   ...
   - `AGENTS.md`, `agent/Codex/`, `agent/cursor/`, `agent/github/` — installed into *consumer*
     repos by `install-skills.sh`, so staleness leaks outward
   - `AGENTS.md`, `doc/MAINTAINERS.md`
...
| something about the *plugin's* architecture, not the update process | `AGENTS.md` |
...
- **Edits apply to the *next* run, not this one.** Codex loads `SKILL.md` once at invocation
