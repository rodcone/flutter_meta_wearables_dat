...
See `doc/MAINTAINERS.md` for detailed steps. Currently on **DAT 0.7.0** on both platforms. Summary:
- **iOS**: Download xcframeworks from github.com/facebook/meta-wearables-dat-ios releases → replace in `ios/Frameworks/` (core: `MWDATCore` + `MWDATCamera`; mock add-on: `MWDATMockDevice`) → `pod update` in example/ios. Skip `MWDATDisplay` and `MWDATMockDeviceTestClient` xcframeworks — not vendored.
