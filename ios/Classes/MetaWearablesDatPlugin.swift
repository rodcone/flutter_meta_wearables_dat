...
public class MetaWearablesDatPlugin: NSObject, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "mockDevicePowerOn":
            ...
            #else
                result(FlutterError(code: "UNAVAILABLE", message: "MWDATMockDevice is not available in this build", details: nil))
            #endif
        ...
        case "grabCurrentFrame":
            grabCurrentFrame(result: result)
    ...
    func startStreamSession(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        NSLog("[MWDAT] startStreamSession — fps=%.1f, quality=%@, device=%@", fps, args["streamQuality"] as? String ?? "high", uuidString ?? "auto")
        ...
        Task { @MainActor in
            ...
            if streamSession != nil {
                if let texId = textureId {
                    NSLog("[MWDAT] startStreamSession — session already active, returning existing texture %lld", texId)
                    result(texId)
                } else {
                    NSLog("[MWDAT] startStreamSession — session exists but no texture")
            ...
            NSLog("[MWDAT] startStreamSession — calling session.start()")
            do {
                await session.start()

                if uuidString == nil, let activeDevice = deviceSelector.activeDevice {
                    NSLog("[MWDAT] startStreamSession — AutoDeviceSelector connected to device: \(activeDevice)")
                }

                NSLog("[MWDAT] startStreamSession — success, returning texture %lld", texId)
                result(texId)
            } catch {
                NSLog("[MWDAT] startStreamSession — failed: %@", error.localizedDescription)
                await cleanupSession()
    ...
    func stopStreamSession(call _: FlutterMethodCall, result: @escaping FlutterResult) {
        NSLog("[MWDAT] stopStreamSession — called, session active: %@", streamSession != nil ? "true" : "false")
        Task { @MainActor in
            guard streamSession != nil else {
                NSLog("[MWDAT] stopStreamSession — no active session")
                result(FlutterError(code: "SESSION_NOT_FOUND", message: "No active stream session", details: nil))
                return
            }

            await cleanupSession()
            NSLog("[MWDAT] stopStreamSession — cleanup complete")
            result(true)
        }
    }

    func capturePhoto(call _: FlutterMethodCall, result: @escaping FlutterResult) {
        Task { @MainActor in
    ...
    func grabCurrentFrame(result: @escaping FlutterResult) {
        guard let texture = pixelBufferTexture else {
            NSLog("[MWDAT] grabCurrentFrame — no pixelBufferTexture")
            result(nil)
            return
        }
        guard let jpegData = texture.copyAsJpegData() else {
            NSLog("[MWDAT] grabCurrentFrame — no pixel buffer available")
            result(nil)
            return
        }
        NSLog("[MWDAT] grabCurrentFrame — returning %d bytes", jpegData.count)
