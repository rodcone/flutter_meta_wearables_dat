...
/// <code>-[MWDATDeviceSession addCameraWithConfig:error:]</code>. The camera owns its child features; use
/// <code>stream</code> for video streaming, and <code>stop()</code> to tear the camera down (cascading to its
/// children and detaching from the session).
SWIFT_CLASS_NAMED("ObjC_Camera")
@interface MWDATCamera : NSObject
/// The video streaming child, owned by this camera.
@property (nonatomic, readonly, strong) MWDATStream * _Nonnull stream;
/// Stops this camera and its child features, releasing resources and detaching from the parent
/// session. Calling <code>stop()</code> on an already-stopped camera is a no-op.
- (void)stop;
...
- (MWDATCamera * _Nullable)addCameraWithError:(NSError * _Nullable * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
/// Creates a consolidated camera capability with the provided stream configuration.
/// Returns <code>nil</code> without setting <code>error</code> when the device session is not started yet.
- (MWDATCamera * _Nullable)addCameraWithConfig:(MWDATStreamConfiguration * _Nonnull)config error:(NSError * _Nullable * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
@end
