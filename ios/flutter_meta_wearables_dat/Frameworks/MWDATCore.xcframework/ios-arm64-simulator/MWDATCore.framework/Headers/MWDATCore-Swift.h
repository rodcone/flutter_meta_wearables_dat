...
SWIFT_CLASS_NAMED("ObjC_DeviceSelector")
@interface MWDATDeviceSelector : NSObject
/// The currently active device identifier, if any.
@property (nonatomic, readonly, copy) NSString * _Nullable activeDevice;
...
/// \param listener A block called with the active device identifier, or
/// <code>nil</code> when no eligible device is selected.
///
///
/// returns:
/// A token that must be retained to keep the listener active.
- (ObjC_AnyListenerToken * _Nonnull)addActiveDeviceListener:(void (^ _Nonnull)(NSString * _Nullable))listener SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
@interface MWDATAutoDeviceSelector : MWDATDeviceSelector
/// Creates a selector that monitors <code>Wearables/shared</code> for devices.
...
@interface MWDATSpecificDeviceSelector : MWDATDeviceSelector
/// Creates a selector that targets a specific device.
/// \param deviceIdentifier The identifier of the device to select.
///
...
/// \param deviceSelector The selector that determines which device to connect to.
///
/// \param error On failure, contains the device session error.
///
///
/// returns:
/// A new device session, or <code>nil</code> if no eligible device is selected.
- (MWDATDeviceSession * _Nullable)createSessionWithDeviceSelector:(MWDATDeviceSelector * _Nonnull)deviceSelector error:(NSError * _Nullable * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
